...
import { adminApi, type FeatureFlag, type AdminRecord, type AuditEvent, ADMIN_ROLES, type AdminRole } from './forgeAdmin';
...
function AdminsTab() {
  ...
  const deactivate = async (id: string) => {
    const reason = window.prompt('Reason for deactivating this admin?');
    if (reason === null) return;
    setFeedback('');
    const res = await adminApi.adminsSet(id, (admins?.find((a) => a.user_id === id)?.role ?? 'support_admin'), false, reason);
    setFeedback(res.ok ? 'Admin deactivated.' : res.message);
    load();
  };
    ...
    <div>
      {canManage && (
        <Card className="mb-5 max-w-2xl">
          <h3 className="text-sm font-semibold text-forge-text-primary mb-3">Add or update an admin</h3>
          ...
          <table className="w-full text-left">
            <thead className="bg-forge-panel-elevated border-b border-forge-border-subtle">
              <tr>
                ...
                <th className="px-3 py-2 text-xs font-medium text-forge-text-muted">Active</th>
                <th className="px-3 py-2 text-xs font-medium text-forge-text-muted">Granted</th>
                {canManage && <th className="px-3 py-2 text-xs font-medium text-forge-text-muted text-right">Action</th>}
              </tr>
            </thead>
            <tbody>
              {admins.map((a) => (
                <tr key={a.user_id} className="border-b border-forge-border-subtle last:border-0 hover:bg-forge-hover/40">
                  <td className="px-3 py-2 text-xs text-forge-text-muted font-mono">{a.user_id.slice(0, 12)}…</td>
                  <td className="px-3 py-2 text-sm text-forge-text-primary capitalize">{a.role.replace(/_/g, ' ')}</td>
                  <td className="px-3 py-2"><StatusPill status={a.active ? 'healthy' : 'disabled'} /></td>
                  <td className="px-3 py-2 text-xs text-forge-text-muted">{formatDate(a.created_at)}</td>
                  {canManage && (
                    <td className="px-3 py-2 text-right">
                      {a.active && <Button size="sm" variant="ghost" onClick={() => deactivate(a.user_id)}>Deactivate</Button>}
                    </td>
                  )}
                </tr>
              ))}
...
function AuditTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError('');
    const res = await adminApi.auditList();
  ...
  return (
    <div>
      {!events || events.length === 0 ? <EmptyState message="No audit events yet." /> : (
        <Card className="p-0 overflow-hidden">
          <table className="w-full text-left">
            <thead className="bg-forge-panel-elevated border-b border-forge-border-subtle">
              <tr>
            ...
            <tbody>
              {events.map((e) => (
                <tr key={e.id} className="border-b border-forge-border-subtle last:border-0 hover:bg-forge-hover/40">
                  <td className="px-3 py-2 text-xs text-forge-text-muted whitespace-nowrap">{formatDate(e.created_at)}</td>
                  <td className="px-3 py-2 text-xs text-forge-text-primary font-mono">{e.action}</td>
                  <td className="px-3 py-2 text-xs text-forge-text-secondary">{e.target_type ? `${e.target_type}${e.target_id ? ':' + e.target_id.slice(0, 8) : ''}` : '—'}</td>
